;;;# Introduction
;;;
;;; KlammerScript is a simple language that looks a lot like Lisp, but
...
;;; expression is a call to the KlammerScript "compiler". The
;;; KlammerScript "compiler" transforms the expression in KlammerScript
...
;;;# Features
;;;
;;; KlammerScript supports all the statements and expressions defined by
...
;;; KlammerScript uses the Lisp reader, allowing for reader macros. It
;;; also comes with its own macro environment, allowing host macros
;;; and KlammerScript to coexist without interfering with each other.
;;; Furthermore, KlammerScript uses its own compiler macro system,
...
;;; KlammerScript allows the creation of JavaScript objects in a Lispy
...
;;; KlammerScript features a HTML generator. Using the same syntax as
...
;;; KlammerScript code, instead of writing the tedious and error-prone
...
;;; Lisp, KlammerScript also provides a sexp-based syntax for CSS
...
;;;# Getting KlammerScript
;;;
;;; KlammerScript can be obtained from the BKNR subversion repository at

    svn://bknr.net/trunk/bknr/src/js

;;; KlammerScript does not depend on any part of BKNR though. You can
;;; download snapshots of KlammerScript at the webpage
...
;;;
;;; After downloading the KlammerScript sourcecode, set up the ASDF
;;; central registry by adding a symlink to "klammerscript.asd". Then
;;; use ASDF to load KlammerScript. You may want to edit the ASDF file
