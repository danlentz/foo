...
;;; This chapters describes the core constructs of KlammerScript, as
;;; well as its compilation model. This chapter is aimed to be a
;;; comprehensive reference for KlammerScript developers. Programmers
;;; looking for how to tweak the KlammerScript compiler itself should
;;; turn to the KlammerScript Internals chapter.
...
;;; KlammerScript forms are expression, but certain special forms are
;;; not (the forms which are transformed to a JavaScript
;;; statement). All KlammerScript expressions are statements
...
;;; KlammerScript programmer to use a practical shortcut when accessing
...
;;; The following keywords and symbols are reserved in KlammerScript,
...
;;;
;;; KlammerScript supports the standard JavaScript literal
...
;;; Indexing arrays in KlammerScript is done using the form `AREF'. Note
...
;;; allows the KlammerScript programmer to be flexible, as flexible as
;;; JavaScript itself.
...
;;; KlammerScript operator is not the assignment operator. Unlike
;;; JavaScript, KlammerScript supports multiple arguments to the
;;; operators.
...
;;; into a JavaScript `for' statement. Note that the KlammerScript `DO'
...
;;; KlammerScript. The default case is not named `T' in KlammerScript, but
;;; `DEFAULT' instead.
...
;;; The HTML generator of KlammerScript is very similar to the HTML
...
;;; KlammerScript can be extended using macros, just like Lisp can be
;;; extended using Lisp macros. Using the special Lisp form
;;; `DEFJSMACRO', the KlammerScript language can be
;;; extended. `DEFJSMACRO' adds the new macro to the toplevel macro
;;; environment, which is always accessible during KlammerScript
...
;;; A more complicated KlammerScript macro example is the implementation
;;; of the `DOLIST' form (note how `JS-GENSYM', the KlammerScript of
;;; `GENSYM', is used to generate new KlammerScript variable names):
...
;;; the KlammerScript `MACROLET' form (note that while `DEFJSMACRO' is a
;;; Lisp form, `MACROLET' and `SYMBOL-MACROLET' are KlammerScript forms).

;;; KlammerScript also supports symbol macros, which can be introduced
;;; using the KlammerScript form `SYMBOL-MACROLET'. A new macro
...
;;; example, the KlammerScript `WITH-SLOTS' is implemented using symbol
;;; macros.
...
;;; The KlammerScript compiler can be invoked from withing Lisp and from
;;; within KlammerScript itself. The primary API function is
;;; `JS-COMPILE', which takes a list of KlammerScript, and returns an
;;; internal object representing the compiled KlammerScript. 
...
;;; interpret the KlammerScript in expression and in statement context
...
;;; As a shortcut, KlammerScript provides the functions `JS-TO-STRING'
...
;;; For static KlammerScript code, the macros `JS', `JS-INLINE',
;;; `JS-FILE' and `JS-SCRIPT' avoid the need to quote the KlammerScript
...
;;; prints the compiled KlammerScript code to the HTML stream.

;;; These macros are also available inside KlammerScript itself, and
;;; generate strings that can be used inside KlammerScript code. Note
;;; that `JS-INLINE' in KlammerScript is not the same `JS-INLINE' form
