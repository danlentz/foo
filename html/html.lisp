...
(defun emit-xml (sexp) (let ((*xhtml* t)) (process (get-pretty-printer) sexp)))
...
(defmacro xml (&whole whole &body body)
  (declare (ignore body))
  `(if *pretty*
     (macrolet ((xml (&body body) (let ((*xhtml* t)) (codegen-html (sexp->ops body) t))))
       (let ((*html-pretty-printer* (get-pretty-printer))
	     (*xhtml* t))
	 ,whole))
     (macrolet ((xml (&body body) (let ((*xhtml* t)) (codegen-html (sexp->ops body) nil))))
       (let ((*xhtml* t))
	 ,whole))))
